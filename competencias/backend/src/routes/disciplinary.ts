...
router.post('/cases', requirePermission('disciplinary.create'), async (req, res) => {
  try {
    ...
    if (!employeeId || !reasonSummary) return res.status(400).json({ error: 'employeeId and reasonSummary are required' });
    const dc = await prisma.disciplinaryCase.create({
      data: {
        ...
        details,
        deadlines,
        openedByUserId: (req as any).user?.sub,
      },
    });
  ...
  try {
    ...
    const charge = await prisma.charge.create({
      data: {
        ...
        createdByUserId: (req as any).user?.sub,
      },
    });
...
router.post('/cases/:id/notify', requirePermission('disciplinary.update'), async (req, res) => {
  try {
    ...
    if (!type || !channel) return res.status(400).json({ error: 'type and channel are required' });
...
router.post('/cases/:id/sanction', requirePermission('disciplinary.update'), async (req, res) => {
  try {
    ...
    if (!type || !justification) return res.status(400).json({ error: 'type and justification are required' });
    const sanction = await prisma.sanction.create({
      data: {
        ...
        decisionByUserId: (req as any).user?.sub,
        justification,
      },
    });
...
router.post('/cases/:id/termination', requirePermission('disciplinary.update'), async (req, res) => {
  try {
    ...
    if (!causeCode || !causeText || !effectiveDate) return res.status(400).json({ error: 'causeCode, causeText, effectiveDate are required' });
