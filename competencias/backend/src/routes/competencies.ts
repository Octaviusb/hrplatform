...
router.get('/gaps/:employeeId', requireAuth, async (req, res) => {
  try {
    ...
    const gaps = requiredCompetencies.map(required => {
      ...
      const requiredLevel = levelMap[required.requiredLevel as keyof typeof levelMap] || 0;
      const currentLevel = current ? levelMap[current.currentLevel as keyof typeof levelMap] || 0 : 0;
