...
router.get('/', requirePermission('payroll.read'), async (req, res) => {
  try {
    const { period, status } = req.query as any;
    const organizationId = (req as any).user.activeOrganizationId;
  ...
  try {
    ...
    for (const payrollId of payrollIds) {
      try {
        ...
        errors.push({ payrollId, error: (error as any)?.message || 'Unknown error' });
      }
    }
...
function validatePayrollForDian(payload: any): { valid: boolean; errors: string[] } {
  const errors: string[] = [];
  if (!payload?.employer?.nit) errors.push('Employer NIT is required');
  if (!payload?.employer?.name) errors.push('Employer name is required');
  if (!payload?.employee?.identification) errors.push('Employee identification is required');
  if (!payload?.employee?.name) errors.push('Employee name is required');
  if (!payload?.payroll?.period) errors.push('Payroll period is required');
  if (!payload?.payroll?.periodStart) errors.push('Payroll periodStart is required');
  if (!payload?.payroll?.periodEnd) errors.push('Payroll periodEnd is required');
  ...
  if (net !== base + bon - ded) errors.push('Net salary must equal base + bonuses - deductions');
